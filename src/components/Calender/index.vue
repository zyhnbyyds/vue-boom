...
<template>
  <div p-3 flex flex-col h-full w-full relative>
    ...
    <div class="flex flex-1 flex-col z-10">
      <div grid="~ cols-7" gap-2 h-10 w-full>
        <div
          v-for="item in 7"
          :key="`${item}:week`"
          class="text-sm border border-light-700 rounded-md flex h-10 items-center justify-center dark:text-gray-300 dark:border-dark-700"
        >
      ...
      <div grid="~ cols-7 rows-5 gap-2" mt-1 flex-1 w-full :class="days.length > 35 ? 'grid-rows-6' : 'grid-rows-5'">
        <div
          ...
          class="p3 border border-light-700 rounded-md transition-colors dark:border-dark-300"
          :class="
            [
              nowDate === `${year}-${month}-${item.day}`
                ...
                : item.isNextMonth || item.isPrevMonth
                  ...
                  : 'hover:bg-light400 hover:bg-op1 bg-op40 cursor-pointer dark:bg-dark-400 dark:hover:bg-dark-200 dark:text-gray-100',
            ]"
        >
    ...
    <div
      v-if="props.showYear"
      class="text-45 text-gray-200 text-stroke-2 text-stroke-gray-400 op30 text-nowrap font-italic left-50% top-50% absolute z-6 dark:text-dark-800 dark:text-stroke-neutral -translate-x-50% -translate-y-50%"
    >
